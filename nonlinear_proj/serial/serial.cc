...
namespace Step15
{
  ...
  private:
    ...
    // Reminder! The next one is required in the parallel version
  ...
  template <int dim>
  void NonLinearProblem<dim>::setup_system (const bool initial_step)
  {
    if (initial_step)
      {
  ...
  void NonLinearProblem<dim>::assemble_system ()
  {
    ...
    std::vector<double>          present_solution_values(n_q_points);
    std::vector<Tensor<1, dim> > solution_gradients(n_q_points);

    Tensor<1,dim> vec_u;
    Tensor<1,dim> vec_u_tilda;
    ...
    for (; cell!=endc; ++cell)
      {
        ...
        fe_values.get_function_values(present_solution,
                                      present_solution_values);
        fe_values.get_function_gradients(present_solution,
                                         solution_gradients);

        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
          {
            vec_u_tilda[0] = present_solution_values[q_point];
            vec_u_tilda[1] = present_solution_values[q_point];
                ...
                for (unsigned int j=0; j<dofs_per_cell; ++j)
                  {
                    ...
                    vec_u[1] = fe_values.shape_value(j, q_point);
  ...
  template <int dim>
  void NonLinearProblem<dim>::solve ()
  {
  ...
  template <int dim>
  void NonLinearProblem<dim>::refine_mesh ()
  {
  ...
  // right value:
  template <int dim>
  void NonLinearProblem<dim>::set_boundary_values ()
  {
    std::map<types::global_dof_index, double> boundary_values;
    VectorTools::interpolate_boundary_values (dof_handler,
                                              0,
  ...
  // operations:
  template <int dim>
  double NonLinearProblem<dim>::compute_residual (const double alpha) const
  {
    const RightHandSide<dim> right_hand_side;
    ...
    for (; cell!=endc; ++cell)
      {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
          {
            vec_u_tilda[0] = solution_values[q_point];
            vec_u_tilda[1] = solution_values[q_point];
  ...
  template <int dim>
  double NonLinearProblem<dim>::determine_step_length() const
  {
    return 0.1;
  }
  ...
  try
    {
      ...
      NonLinearProblem<2> laplace_problem_2d;
