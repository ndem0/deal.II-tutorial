...
namespace Step40
{
      ...
      if (cell->is_locally_owned())
        {
          ...
          fe_values.get_function_values(present_solution, present_solution_values);
          fe_values.get_function_gradients(present_solution, solution_gradients);
  ...
  template <int dim>
  void LaplaceProblem<dim>::compute_residual (const double alpha) const
  {
    TimerOutput::Scope t(computing_timer, "assembly");
    ...
    Vector<double>               cell_residual (dof_handler.n_locally_owned_dofs());
    Vector<double>               evaluation_point (dof_handler.n_locally_owned_dofs());
  ...
  void LaplaceProblem<dim>::solve ()
  {
    ...
    /* TODO at the end of the non linear newton iteration
